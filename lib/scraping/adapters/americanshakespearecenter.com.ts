...
export async function scrapeASC(): Promise<NormalizedEvent[]> {
  const events: NormalizedEvent[] = []
  const baseUrl = 'https://americanshakespearecenter.com'
  
  try {
    console.log('🎭 Scraping American Shakespeare Center...')
    
    const mainPageResult = await fetchWithPoliteness(baseUrl)
    if (mainPageResult.status !== 200) {
      console.error(`Failed to fetch ASC main page: ${mainPageResult.status}`)
      return events
    }
    ...
    console.log(`✅ ASC scraping completed. Found ${events.length} total events.`)
  } catch (error) {
    console.error('ASC scraping failed:', error)
  }
