...
export default async function PlayDetailPage({ params }: { params: { play: string } }) {
  ...
  const byCompany = productions.reduce<Record<string, typeof productions>>((acc: Record<string, typeof productions>, p: typeof productions[number])=>{(acc[p.companyId]=acc[p.companyId]||[]).push(p);return acc}, {})
          ...
          return (
            <section key={cid}>
              ...
              <ul className="divide-y">
                {list.map((p: typeof productions[number]) => (
