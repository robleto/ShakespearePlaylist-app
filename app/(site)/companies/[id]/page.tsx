...
export default async function CompanyDetail({ params }: { params: Params }) {
  ...
  const byPlay = productions.reduce<Record<string, typeof productions>>((acc: Record<string, typeof productions>, p: typeof productions[number])=>{(acc[p.canonicalPlay]=acc[p.canonicalPlay]||[]).push(p);return acc}, {})
        ...
        {playKeys.map(pk => (
          <section key={pk}>
            ...
            <ul className="divide-y">
              {byPlay[pk].map((p: typeof productions[number]) => (
